...
def main():
    ...
    eobs_url = raw_url(CH10_REPO, CH10_SHA, f"{BASE_PATH}/stations_info_tg_v21.0e.txt")
    donat_url = raw_url(CH10_REPO, CH10_SHA, f"{BASE_PATH}/Donat_Stations_tas.csv")

    eobs = pd.read_csv(
        eobs_url,
        sep="|",
        comment="#",
        names=["STATION", "NAME", "COUNTRY", "LAT", "LON", "ELEV", "START", "STOP"],
        engine="python",
    )
    ...
    for col in ("LAT", "LON"):
        eobs[col] = pd.to_numeric(eobs[col], errors="coerce")
        donat[col] = pd.to_numeric(donat[col], errors="coerce")
    ...
    with publication_context(width="single", height_mm=72, strict_font=False):
        ...
        ax.coastlines(resolution="110m", linewidth=0.5, color="black", zorder=2)
        ax.add_feature(
            cfeature.BORDERS,
            facecolor="none",
            edgecolor="#808080",
        ...
        return finalize(
            ...
            metadata={
                "Subject": (
                    "Reproduction of AR6 WGI Chapter 10 Figure 10.20b station-location "
                    "map from pinned official ESMValTool source data"
                ),
            },
        )
